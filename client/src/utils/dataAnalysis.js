...
/**
 * Parse CSV into javascript array
 * @param {string} filePath 
 * @param {int} years: number of years of data to analyze 
 */
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    csv()
      ...
      .on('json', (jsonObj) => {
        ...
        const price = +jsonObj.Price.split(',').join('');
        parsedCSV.push({ date, price });
      })
      .on('done', (error) => {
...
const getYtdReturn = (values) => {
  ...
  const year = values[values.length - 1].date.getFullYear();
  while (values[index].date.getFullYear() === year && index > 0) {
    index -= 1;
  }
...
const getMonthlyReturns = (values) => {
  ...
  let date1_ms = values[index1].date.getTime();
  let date2_ms = values[index2].date.getTime();
  
  while (index2 < values.length) {
    while ((date2_ms - date1_ms) < thirtyDays) {
      index2 += 1;
      if (index2 < values.length) {
        date2_ms = values[index2].date.getTime();
      } else {
        date2_ms += 1000000;
      }
    }
    ...
    index1 += 1;
    date1_ms = values[index1].date.getTime();
  }
...
const summaryStats = (filePathObj, startDate, endDate) => {

  console.log('*****', startDate, endDate);

  // Data inputs
  const assetFilePath = filePathObj.filepath;
  // const rfFilePath = fedFundsFilePath;
  // const years = 3;

  // Process CSV promises
  const assetPromise = processCSV(assetFilePath);
  // const rfPromise = processCSV(rfFilePath);

  Promise.all([assetPromise]) // , rfPromise
    .then(([asset]) => { // , rfRates

      console.log();
      console.log('== ', filePathObj.asset, ' ==');
